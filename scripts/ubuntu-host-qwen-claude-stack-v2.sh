...
wait_for_http() {
  local url="$1" header_arg="${2:-}" attempts="${3:-30}" delay="${4:-1}" i
  for ((i=1; i<=attempts; i++)); do
    ...
    printf "%s" "${DIM}.${RESET}"; sleep "$delay"
  done
  echo; return 1
}
...
install_litellm() {
  ...
  if [[ "$FORCE_REINSTALL_LITELLM" == "1" && -d "$VENV_DIR" ]]; then log WARN "Removing existing venv because FORCE_REINSTALL_LITELLM=1"; rm -rf "$VENV_DIR"; fi
...
HOST_CONFIG_PATH="${HOST_CONFIG_PATH:-$(pwd)/macos-host-preconfig.env}" ./scripts/macos-vm-claude-client.sh install
MACBOOTEOF
...
probe() {
  step "Health checks"
  local ollama_url="http://127.0.0.1:$OLLAMA_PORT/api/tags" litellm_url="http://127.0.0.1:$LITELLM_PORT/v1/models" response=""
  ...
  if command_exists jq; then
    response="$(curl -fsS "$ollama_url/../generate" -H "Content-Type: application/json" -d "{\"model\":\"$MODEL_ALIAS\",\"prompt\":\"Return exactly LOCAL_QWEN_READY and nothing else.\",\"stream\":false}" | jq -r '.response' || true)"
  else
    response="$(curl -fsS "$ollama_url/../generate" -H "Content-Type: application/json" -d "{\"model\":\"$MODEL_ALIAS\",\"prompt\":\"Return exactly LOCAL_QWEN_READY and nothing else.\",\"stream\":false}" | python3 -c 'import sys,json; print(json.load(sys.stdin).get("response", ""))' 2>/dev/null || true)"
  fi
...
print_mac_env() { print_env; echo; echo "# Copy to macOS VM:"; echo "#   scp '$MAC_PRECONFIG' user@mac-vm:~/macos-host-preconfig.env"; echo "#   HOST_CONFIG_PATH=~/macos-host-preconfig.env ./scripts/macos-vm-claude-client.sh install"; }
...
restart_stack() { step "Restart stack"; if systemd_available && systemctl list-unit-files 2>/dev/null | grep -q '^ollama.service'; then require_sudo; run sudo systemctl restart ollama; fi; if systemd_user_available; then run systemctl --user restart "$SERVICE_NAME" || true; else stop_pidfile "$OLLAMA_PIDFILE" "Ollama"; stop_pidfile "$LITELLM_PIDFILE" "LiteLLM"; configure_ollama_service; start_litellm_service; fi; probe; }
stop_stack() { step "Stop stack"; if systemd_user_available; then run systemctl --user stop "$SERVICE_NAME" || true; fi; stop_pidfile "$LITELLM_PIDFILE" "LiteLLM"; stop_pidfile "$OLLAMA_PIDFILE" "Ollama"; log OK "Tracked fallback processes stopped"; }
...
summary() {
  ...
  echo; echo "${CYAN}${BOLD}macOS VM command:${RESET}"; echo "  HOST_CONFIG_PATH=$MAC_PRECONFIG ./scripts/macos-vm-claude-client.sh install"; echo
}
