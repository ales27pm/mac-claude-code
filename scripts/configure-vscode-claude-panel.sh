...
case "$CLAUDE_BACKEND" in
  ollama)
    ...
    ;;
  litellm)
    CLAUDE_BASE_URL="$LITELLM_BASE_URL"
    CLAUDE_AUTH_TOKEN="$LITELLM_KEY"
    CLAUDE_API_KEY="$LITELLM_KEY"
    ;;
  *)
    echo "CLAUDE_BACKEND must be 'ollama' or 'litellm', got: $CLAUDE_BACKEND" >&2
    exit 2
    ;;
esac
...
log INFO "Then open the Claude panel. It should use the Anthropic-compatible endpoint above."
